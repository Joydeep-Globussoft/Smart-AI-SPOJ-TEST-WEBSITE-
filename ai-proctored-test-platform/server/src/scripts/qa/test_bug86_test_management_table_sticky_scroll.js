...
runTest('AdminTests.jsx has single table scroll container with sticky header and bottom scrollbar', () => {
  assert(adminTestsContent.includes('test-table-scroll-container'), 'Table must use test-table-scroll-container');
  assert(!adminTestsContent.includes('persistent-horizontal-scrollbar'), 'Must not render secondary duplicate scrollbar track');
  assert(adminTestsContent.includes('minWidth: 1400') || adminTestsContent.includes('minWidth: 1250') || adminTestsContent.includes('minWidth: 1140') || adminTestsContent.includes('minWidth: 1060') || adminTestsContent.includes('minWidth: 980') || adminTestsContent.includes('minWidth: 960'), 'Table must have minWidth to prevent column crushing');
});
...
runTest('AdminTests.jsx renders folder-linked and manual Question Sets as plain text (BUG-87)', () => {
  assert(
    adminTestsContent.includes('questionSetName') || adminTestsContent.includes('{test.questionSetPoolName || test.folderId?.name || test.questionSetId?.name || \'—\'}'),
